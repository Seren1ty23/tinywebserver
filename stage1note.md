...
- **参数**：`fd` 文件描述符；`buf` 用户态缓冲区（数据读到这里）；`count` 最多读多少字节
- **返回值**：
  - `> 0`：实际读到的字节数（**可能小于 count**，不一定一次读满）
  - `== 0`：读到文件末尾（EOF），没数据了 → 退出循环
  - `-1`：出错，看 errno（常见：`EAGAIN`=非阻塞下暂时没数据、`EINTR`=被信号打断）
- **关键**：返回值可能 < count，所以要**循环读**，直到返回 0
...
- **参数**：`fd` 文件描述符；`buf` 要写的数据；`count` 写多少字节
- **返回值**：
  - `> 0`：实际写入的字节数（**可能小于 count，即「部分写入」**）
  - `-1`：出错，看 errno
- **关键**：可能「部分写入」→ 要**循环写**，用偏移量 `off` 追平（见参考实现）

### 阻塞 vs 非阻塞（阶段 5 的核心概念）

- **阻塞**（默认）：`read` 没数据就**一直等**；`write` 缓冲区满也等
- **非阻塞**（`fcntl` 设 `O_NONBLOCK`）：立即返回，没数据时 `read` 返回 -1 且 `errno == EAGAIN`
- `EAGAIN`（= `EWOULDBLOCK`）**不是错误**，表示「暂时没数据，稍后再试」——这是 epoll 阶段的关键
